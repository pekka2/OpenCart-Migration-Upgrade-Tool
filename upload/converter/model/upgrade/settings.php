  ...
  public function addSetting( $data ){
   ...
   private function newSettings(){
	...
	if( !$this->structure->hasSetting( 'config_checkout_guest' ) ) {
		...
		++$this->settingcounter;
		$text .= $this->msg( sprintf( $this->lang['msg_config'], 'config_checkout_guest', '' ) );
	}
	...
	if( !$this->structure->hasSetting( 'config_order_mail' ) ) {
		...
		++$this->settingcounter;
		$text .= $this->msg( sprintf( $this->lang['msg_config'], 'config_order_mail', '' ) );
	}
	...
	if( !$this->structure->hasSetting( 'config_image_compare_width' ) ) {
		$sql = '
		INSERT INTO
			`' . DB_PREFIX . 'setting`
		SET
		...
		++$this->settingcounter;
		$text .= $this->msg( sprintf( $this->lang['msg_config'], 'config_image_compare_width', 'setting' ) );
	}


	if( !$this->structure->hasSetting( 'config_image_compare_height' ) ) {
		$sql = '
		INSERT INTO
			`' . DB_PREFIX . 'setting`
		SET
		...
		++$this->settingcounter;
		$text .= $this->msg( sprintf( $this->lang['msg_config'], 'config_image_compare_height', 'setting' ) );
	}


	if( !$this->structure->hasSetting( 'config_image_wishlist_width' ) ) {
		$sql = '
		INSERT INTO
			`' . DB_PREFIX . 'setting`
		SET
			`store_id` = \'0\',
			`code` = \'config\',
			`key` = \'config_image_wishlist_width\',
			`value` = \'47\',
			`serialized` = \'0\'';
		...
		++$this->settingcounter;
		$text .= $this->msg( sprintf( $this->lang['msg_config'], 'config_image_wishlist_width', 'setting' ) );
	}

 
	if( !$this->structure->hasSetting( 'config_image_wishlist_height' ) ) {
		$sql = '
		INSERT INTO
			`' . DB_PREFIX . 'setting`
		SET
			`store_id` = \'0\',
			`code` = \'config\',
			`key` = \'config_image_wishlist_height\',
			`value` = \'47\',
			`serialized` = \'0\'';
		...
		++$this->settingcounter;
		$text .= $this->msg( sprintf( $this->lang['msg_config'], 'config_image_wishlist_height', 'setting' ) );
	}
