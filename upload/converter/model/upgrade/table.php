...
class ModelUpgradeTable extends Model{
  ...
  public function addUpgradeTo150() {  
	...
	if( !array_search( DB_PREFIX . 'attribute' , $this->structure->tables()) ) {
		...
		$sql = '
		  ...
		  $text .= $this->msg( sprintf( $this->lang['msg_text'],   'attribute', $this->lang['msg_new_data'] ) );
	...
	if( !array_search( DB_PREFIX . 'attribute_description' , $this->structure->tables()) ) {
		...
		if( !$this->simulate ) {
		  ...
		  $text .= $this->msg( sprintf( $this->lang['msg_text'],   'attribute_description', $this->lang['msg_new_data'] ) );
	...
	if( !array_search( DB_PREFIX . 'attribute_group' , $this->structure->tables()) ) {
		...
		if( !$this->simulate ) {
		  ...
		  $text .= $this->msg( sprintf( $this->lang['msg_text'],   'attribute_group', $this->lang['msg_new_data'] ) );
	...
	if( !array_search( DB_PREFIX . 'attribute_group_description' , $this->structure->tables()) ) {
		...
		if( !$this->simulate ) {
		  ...
		  $text .= $this->msg( sprintf( $this->lang['msg_text'],   'attribute_group_description', $this->lang['msg_new_data'] ) );
	...
	if( !array_search( DB_PREFIX . 'voucher_theme' , $this->structure->tables()) ) {
		...
		$sql = '
					INSERT INTO `' . DB_PREFIX . 'voucher_theme` (`voucher_theme_id`, `image`) VALUES
					(8, \'\'),
					(7, \'\'),
					(6, \'\');';   
					if( !$this->simulate ) {
  ...
  public function addUpgradeTo200() {
        $text = '';
  ...
  public function addUpgradeTo202() {
	...
	if( $this->upgrade > 203){
		...
		$text .= $this->version( sprintf( $this->lang['msg_upgrade_to_version'],   '2.0.2 - 2.0.3', '' ) );
    return $text;
  }
  public function addUpgradeTo204() {
        $text = '';
	...
	if( !array_search( DB_PREFIX . 'api_session', $this->structure->tables() ) ) {
		...
		$text .= $this->version( sprintf( $this->lang['msg_upgrade_to_version'],   '2.0.4.0', '' ) );
    return $text;
  }
